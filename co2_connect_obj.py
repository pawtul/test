...
import numpy as np
import time
from utilities import clean_text, fraction_replace, Quantity, find_words
...
attributes = pd.read_csv(PATH + '\\attributes.csv').fillna('').values.tolist()
filler = pd.read_csv(PATH + '\\filler_strings.csv').fillna('').values.tolist()

weights = pd.read_csv(PATH + '\\weights.csv').fillna('').values.tolist()
...
reference_lists = {}
reference_lists['de'] = pd.read_csv(PATH + '\\reference_list_de.csv')
reference_lists['it'] = pd.read_csv(PATH + '\\reference_list_it.csv')
...
def co2_add(recipe_json):
    ...
    try:
        ...
        lang = input('Please specify the recipes language: ').lower()
    ...
    for c in conv:
        for m in c[1].split(','):
            meas.append(m.strip().replace('.', '\.'))
    all_units = ('|'.join(meas))
    
    # get list of filler strings
    filler_list = []
    for f in filler:
        filler_list.append(f[lang_code])
    
    filler_reg = r'\b({})\S?\b'
    filler_reg = filler_reg.format('|'.join(filler_list))

    ## ------------------------------------------------------------------------
    nomatchcount = 0
    noquantitycount = 0
    
    # print(recipe_json['recipe_id'], recipe_json['title'])
    ...
    for r in recipe_json['ingredients']:
        ...
        qty = fraction_replace(r['quantity'])
        qty_orig = qty
        ing = fraction_replace(r['ingredient'])
        ing_orig = r['ingredient']
        ...
        attribute = None
        for a in attributes:
            if a[lang_code] in ing:
                attribute = a[0]
                ing_attributes.append(attribute)
        ...
        quantity_object_qty = Quantity(all_units, qty)
        ...
        if qty == None:
            quantity_object_ing = Quantity(meas, ing)
        ...
        # for ing_part in ing_split: 
        #     full_list['score'] = full_list.apply(lambda row : (fuzz.token_sort_ratio(row['ref'], ing_part)*0.5
        #                                          +fuzz.token_set_ratio(row['ref'], ing_part)*0.5)/100, axis = 1)
        #     df_temp = df_temp.append(full_list)
        ...
        for scr in range(20):
            ...
            if find_words(ing_orig, final_df.iloc[scr].ref):
                ...
                if match.ref == final_df.iloc[scr+1].ref:
                    ...
                    if len(co2_calc) > 1:
                        match.co2 = round(sum(co2_calc) / len(co2_calc), 5)
                        warnings.append('avg')
                    break
        ...
        if qty != None and 'item' in qty:
            try:
                for w in weights:
                   if ingre == w[0]:
                       ingweight = round(w[3] * count, 5)
            except: 
                pass
        ...
        if ingweight != None and noco2 is False:
        ...
        r['ingredient_lang_clean'] = match.lang_key
        r['weight'] = ingweight
        r['co2kg'] = co2kg
...
recipe = {'recipe_id': 1,
  ...
  'ingredients': [{'ingredient': 'Gänsekäule', 'quantity': 'etwas'},
                  {'ingredient': 'Ente', 'quantity': '2 kg'}],
  'language': 'de'}
